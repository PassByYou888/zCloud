...
var
  ...
  i: Integer;
begin
  with TDTC40_PhysicsService.Create(AddrEdit.Text, umlStrToInt(PortEdit.Text), PhysicsIO.TPhysicsServer.Create) do
    begin
      AutoFreePhysicsTunnel := True;
      BuildDependNetwork(ServiceTypeEdit.Text);
      StartService;
    end;
  arry := DTC40_ServicePool.GetFromServiceTyp('var');
  for i := 0 to length(arry) - 1 do
      TDTC40_Var_Service(arry[i]).OnRemove := Do_NMPool_Remove;
end;
...
begin
  ...
  if length(arry) = 0 then
      exit;
  TDTC40_Var_Service(arry[0]).NMBigPool.ProgressP(procedure(const NMPoolName_: PSystemString; NMPool_: TDTC40_VarService_NM_Pool)
    begin
